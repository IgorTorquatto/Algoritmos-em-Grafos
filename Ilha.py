...
class Ilha:
    ...
    def distribuir_inimigos(self):
        ...
        while qtd_inimigos < 5:  # Distribuir 5 inimigos
            ...
            if indice_vertice_aleatorio not in self.objetos_por_vertice and indice_vertice_aleatorio not in vertices_utilizados:
                inimigo = random.choice([Onca(), Crocodilo(), Formiga()])
                self.objetos_por_vertice[indice_vertice_aleatorio] = [inimigo]
    ...
    def distribuir_plantas(self):
        ...
        while qtd_plantas < 5:  # Distribuir 5 plantas
            ...
            if indice_vertice_aleatorio not in self.objetos_por_vertice and indice_vertice_aleatorio not in vertices_utilizados:
                planta = Planta()
                self.objetos_por_vertice[indice_vertice_aleatorio] = [planta]
                vertices_utilizados.add(indice_vertice_aleatorio)
                qtd_plantas += 1
                self.qtd_plantas+=1
        #print(qtd_plantas)
    ...
    def obter_descricao_vertice(self):
        ...
        if vertice_atual in self.objetos_por_vertice:
            recurso = self.objetos_por_vertice[vertice_atual][0]  # Acessar o primeiro item da lista
            descricao = recurso.descricao
            return descricao
        else:
            return "Nenhum recurso neste vértice"

    def remover_objeto_do_vertice(self, posicao_vertice, objeto):
        if posicao_vertice in self.objetos_por_vertice:
            if objeto in self.objetos_por_vertice[posicao_vertice]:
                self.objetos_por_vertice[posicao_vertice].remove(objeto)
